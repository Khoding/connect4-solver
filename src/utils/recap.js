...
export function classifyPly(played, best) {
  if (played == null || best == null || Number.isNaN(played) || Number.isNaN(best))
    return 'unknown';
...
export function recapToText(recap) {
  ...
  lines.push(
    `${pad('Ply', 4)} ${pad('By', 3)} ${pad('Col', 4)} ${pad('Score', 6)} ${pad('Best', 6)}  Quality`,
  );
  ...
  lines.push('');
  lines.push('Generated by connect4-solver · https://connect4-solver.com');
...
export function recapCardSvg(recap) {
  ...
  plies.forEach((p, i) => {
    ...
    parts.push(
      `<rect x="${x.toFixed(2)}" y="${y}" width="${segW.toFixed(2)}" height="26" rx="3" fill="${TIER_META[p.quality].color}"/>`,
    );
  });
  y += 26 + 28;
  ...
  /* Footer */
  parts.push(
    `<line x1="${P}" y1="${y}" x2="${W - P}" y2="${y}" stroke="${THEME.border}" stroke-width="1"/>`,
  );
  y += 20;
  parts.push(
  ...
  parts.push(
    `<text x="${W - P}" y="${y}" text-anchor="end" font-family="${FONT}" font-size="12" fill="${THEME.dim}">connect4-solver · https://connect4-solver.com</text>`,
  );
  y += P;
